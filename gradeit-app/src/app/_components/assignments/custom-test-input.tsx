...
export function CustomTestInput({ input, onInputChange, output, isRunning }: CustomTestInputProps) {
  return (
    <div className="h-80 overflow-y-auto bg-[#1E1E1E] text-white">
      <div className="p-4">
        <div className="mb-4">
          <label htmlFor="custom-input" className="mb-2 block text-sm font-medium text-[#A1A1A1]">
            Input
          </label>
          <Textarea
            id="custom-input"
          ...
          <p className="mt-1 text-xs text-[#A1A1A1]">
            Enter input values separated by line breaks or spaces as required by your code.
          </p>
        </div>

        {output !== null && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
            <label className="mb-2 block text-sm font-medium text-[#A1A1A1]">Output</label>
