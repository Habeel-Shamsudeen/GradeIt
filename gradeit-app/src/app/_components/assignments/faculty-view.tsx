...
import { Search, Filter, Download, Users, CheckCircle, Clock, XCircle } from "lucide-react"
...
export function FacultyView({ assignment, classId }: FacultyViewProps) {
  ...
  return (
    <div className="container mx-auto max-w-7xl p-6">
      <div className="mb-8">
        <h1 className="text-2xl font-medium text-[#141413]">{assignment.title}</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-4">
        ...
        <Card className="rounded-2xl border-[#E6E4DD]">
          <CardHeader>
            ...
            <CardDescription>Students who completed all questions</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardContent>
            <div className="divide-y divide-[#E6E4DD]">
                  ...
                  <div className="flex items-center gap-6">
                    <div className="text-right">
                      <p className="text-sm font-medium text-[#141413]">
                        {student.questionsCompleted} / {assignment.questions.length} Questions
                      </p>
                      {student.status === "completed" && (
                        <p className="text-sm text-[#605F5B]">Score: {student.score}%</p>
                      )}
                    </div>
                    <div
                      className={cn(
                        "flex h-8 items-center rounded-full px-3 text-sm",
                        student.status === "completed" && "bg-[#7EBF8E]/10 text-[#7EBF8E]",
                        student.status === "in_progress" && "bg-[#F1E6D0]/10 text-[#3A3935]",
                        student.status === "not_started" && "bg-[#D2886F]/10 text-[#D2886F]",
                      )}
                    >
                    ...
                    <Button variant="ghost" size="sm" className="h-8 px-2 text-[#605F5B]">
                      View Details
                    </Button>
                  </div>
                </motion.div>
              ...
              {filteredStudents.length === 0 && (
                <div className="py-8 text-center text-[#605F5B]">No students found matching your search</div>
              )}
            </div>
          </CardContent>
        </Card>
